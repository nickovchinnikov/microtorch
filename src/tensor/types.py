...
from src.tensor.backend.types import (
    Axis,
    Dims,
    DType,
    Index,
...
Data = Scalar | Vector | list | Self

@dataclass(frozen=True)
class Leaf(Generic[Self]):
    r"""A leaf node in the computational graph.

    Attributes:
        value (TensorLike): The tensor value at the leaf.
        grad_fn (Callable[[Vector], Vector]): Function to compute
        the gradient for this node.
    """
    value: "TensorLike"
    grad_fn: Callable[[Vector], Vector]


DependenciesList = list[Leaf[Self]]
...
class TProps:
    ...
    def props(self) -> tuple[Vector, bool, DependenciesList, Device, DType]:
        return (
            self._data,
            self.requires_grad,
            self.dependencies,
...
class TensorLike(Protocol):
    ...
    def to(self, device: Device | str, dtype: DType = None) -> Self: ...
    def zero_grad(self): ...
    def release_grad(self) -> None: ...
    def backward(self, grad: Data | None = None): ...
    ...
    def transpose(self: Self, axis: Axis | None) -> Self: ...
    def squeeze(self: Self, axis: Axis | None) -> Self: ...
    ...
    # --- Reduction Ops ---
    def sum(self: Self, axis: Axis | None, keepdims: bool = False) -> Self: ...
    def mean(self: Self, axis: Axis | None, keepdims: bool = False) -> Self: ...
    def min(self: Self, axis: Axis | None, keepdims: bool = False) -> Self: ...
    def max(self: Self, axis: Axis | None, keepdims: bool = False) -> Self: ...
    def logsumexp(self: Self, axis: Axis | None, keepdims: bool = False) -> Self: ...
